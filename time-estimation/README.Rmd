...
index <- which(rho == max(rho))
lambda_max <- lambda[index]
mu_max <- mu[index]
S <- 5

metrics <- data.frame (
  Lq = Lq(S, lambda_max, mu_max),
  Wq = Wq(S, lambda_max, mu_max) * 3600,
  P0 = p0_(S, lambda_max, mu_max) * 100,
  PD = pd(S, lambda_max, mu_max) * 100,
  rho = lambda_max / mu_max / S * 100
)
```
In a previous section it was shown that the highest utilization of the _Alberti_ toll booth plaza happens during the interval of time between `r index` and `r index + 1`, with a percentage of utilization $\rho$ of `r round(max(rho), 2)`%. That number is obtained by computing $\rho = \frac{\lambda}{S\mu}$. For this particular set of metric values, the average length of the queue $L_q$ would be `r round(metrics$Lq, 2)` vehicles, the average time spent in a queue $W_q$ would be `r round(metrics$Wq, 2)` seconds, the probability of arriving at an empty queue $P_0$ would be `r round(metrics$P0, 2)`%, and the probability of having to wait to go throught he toll booth $P_d$ would be `r round(metrics$PD, 2)`%, for the mentioned utilization of `r round(max(rho), 2)`%.
...
characteristics <- function (
  ...
  c (
    ...
    P0 = p0_(S, lambda_max, mu_max) * 100,
    PD = pd(S, lambda_max, mu_max) * 100,
    rho = min(lambda_max / mu_max / S, 1) * 100
  )
}
...
metrics <- data.frame (
  hour = 0:23,
  servers_req = ceiling(lambda / mu)
)

ggplot(data = metrics, 
       aes(x = hour, y = servers_req)) +
  geom_line(linetype = "dashed") +
  geom_point(color = "blue") +
  labs(x = "Hour of day", 
       y = "Amount of servers",
...
mu <- share %>%
  ...
  summarise (
    amount = 1 / (automatic * t_automatic + manual * t_manual)
  )

rho <- data.frame (
  toll_booth_name = lambda$toll_booth_name,
  start_hour = lambda$start_hour,
  lambda = lambda$amount,
  mu = mu$amount,
  booths = lambda$booths,
  rho = lambda$amount / (lambda$booths_number * mu$amount)
)
...
ggplot(data = rho,
  ...
  labs(x = "Hour of day",
       ...
       title = 
         "Hourly utilization for all toll booth plazas",
       caption = "Figure 12")
```
...
toll_booth_max_req <- metrics %>%
  group_by (
    toll_booth_name
  ) %>%
  summarise (
    amount = max(servers_req)
  )

toll_booth_actual <- rho %>%
  group_by (
    toll_booth_name
  ) %>%
  summarise (
...
data.frame (
  name = toll_booth_actual$toll_booth_name,
  actual = toll_booth_actual$amount,
  minimum = toll_booth_max_req$amount,
  difference = toll_booth_actual$amount - toll_booth_max_req$amount
)
```


```{r removeall, echo=FALSE}
rm(list = ls()); gc();
```
