...
The average length of a non-empty queue is computed as $E(m|m > 0) = \frac{1}{1 - \frac{\rho}{N}}$, and for the _Avellaneda_ toll booth plaza would yield an average length of `r round(em_gt0(S, rho), 2)` cars. By using this expression we can know the average number of units in the system, which results from $E(n) = \rho + E(m) = 0.9 + 1.03 = 1.93$ cars. 
