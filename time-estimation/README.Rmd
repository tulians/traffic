...
# Not used in this README. Maps to Figure 5 in the formal essay.
# share_tall <- share %>%
#   filter (
#     toll_booth_name == "Alberti"
#   ) %>%
#   gather (
#     PaymentMethod, Percentage, manual:automatic
#   )
# 
# ggplot(data = share_tall, aes(x = Percentage, fill = PaymentMethod)) +
#   geom_density(alpha = .3) +
#   labs(x = "Porcentaje de uso",
#        y = "Densidad")
...
# Not used in this README. Maps to Figure 8 in the formal essay.
# mu_f <- mu %>%
#   filter (
#     !toll_booth_name %in% c("Salguero", "Sarmiento")
#   )
# 
# ggplot(data = mu_f, 
...
#   geom_point(aes(color = toll_booth_name)) +
#   labs(x = "Hora del día", y = "Cantidad de vehículos servidos")
...
# rho_t <- rho %>%
#   filter (
#     toll_booth_name == "Salguero"
#   )
# 
# index <- which(rho_t$utilization == max(rho_t$utilization))
# lambda_max <- rho_t$arrival[index]
# mu_max <- rho_t$service[index]
# S <- mean(rho_t$booths)
# 
# metrics <- data.frame (
#   do.call("rbind", lapply(1:4, characteristics, lambda_max, mu_max))
# )
# metrics
```
...
Utilization can be more easily visualized with the heatmap presented in Figure 13.

```{r heatmap, echo=FALSE}
ggplot(data = rho, aes(x = substr(start_hour, 1, 2), y = toll_booth_name)) +
  geom_tile(aes(fill = utilization), color = "white") +
...
rm(servers_required, servers_required_max, servers_required_current,
   servers_required_alberti, characteristics, rho)
```

### Oil prices analysis
The oil prices dataset consists of monthly prices of different types of oil: super, premium, gasoil, and euro. The four of them experienced a steady increase since 2008 until 2019, which is illustrated in Figure 15.
...
oil_yoy_trend_dollar <- data.frame (
  ...
  price = oil_yoy_trend$price / monthly_rate$amount
)
...
Taking differences of the ratio between oil price and ARS/USD exchange rate, Figure 17 shows there are two interesting points in time, where the oil price drops, comparing 2015 with 2016, and afterwars increases, comparing 2016 with 2017.
...
However, these differences are in the order of cents, as the prices vary at most 20c. The stability of oil price expressed in dollars is illustrated in Figure 18.
...
rm(monthly_rate, oil_increase_diff, oil_yoy_trend, oil_yoy_trend_dollar)
```

```{r monthlyseries, echo=FALSE}
monthly_vehicles <- unified_ff %>% 
  as_tibble() %>%
  filter (
    (year_ >= 2015) & (year_ < 2019)
  ) %>%
  group_by (
    year_,
    month_
...
monthly_values <- data.frame (
  year = monthly_vehicles$year_,
  month = monthly_vehicles$month_,
  ym = ymd(paste0(monthly_vehicles$year_, "-", monthly_vehicles$month_, "-01")),
  vehicles = monthly_vehicles$amount,
  rate = monthly_rate$pesos_per_dollar,
  price = monthly_vehicles$price / monthly_rate$pesos_per_dollar
)

ggplot(data = monthly_values, aes(x = ym, group = 1)) +
  geom_line(aes(y = vehicles, color = "Vehicles")) +
  geom_line(aes(y = price * 1e7, color = "Oil price")) +
  scale_y_continuous (
    sec.axis = sec_axis(~./1e7, name = "Oil price (USD)")
  ) +
  scale_colour_manual(values = c("blue", "red")) +
  labs(y = "Amount of vehicles",
       x = "Date",
       color = "Parameter",
       title = "Evolution of oil price and vehicles through the months",
       caption = "Figure 19") + 
  theme(axis.text.x = element_text(angle = 90))
```

```{r monthlyseriesdiff, echo=FALSE}
#' Evaluate whether an increase in series_a at time t resulted in an increase
#' in series_b at time t + n.
#'
#' @param series_a Series to apply the lag to.
#' @param series_b Series to match the other series to.
#' @param lag_n Lag value to apply to series_a.
#'
#' @return Average amount of times an increase in series_a in time t generated
#'         an increase in series_b in t + lag_n
#'
#' @examples lag_and_match(diff(x_1), x_2, 1)
lag_and_match <- function (
  series_a,
  series_b,
  lag_n
) {
  series_a <- lag(x = series_a, n = lag_n)
  (series_a > 0) == (series_b < 0)
}

monthly_values_diff <- data.frame (
  year = monthly_values$year[-1],
  month = monthly_values$month[-1],
  ym = monthly_values$ym[-1],
  vehicles = diff(monthly_values$vehicles),
  price = diff(monthly_values$price),
  drop = lag_and_match (
    diff(monthly_values$price),
    diff(monthly_values$vehicles),
    1
  )
)

ggplot(data = monthly_values_diff, aes(x = ym, group = 1)) +
  geom_line(aes(y = vehicles, color = "Vehicles")) +
  geom_line(aes(y = lag(price * 1e7, n = 1), color = "Oil price")) +
  scale_y_continuous (
    sec.axis = sec_axis(~./1e7, name = "Oil price (USD)")
  ) +
  scale_colour_manual(values = c("blue", "red")) +
  labs(y = "Amount of vehicles",
       x = "Date",
       color = "Parameter",
       title = "Relation of variations in oil price and vehicle volume",
       caption = "Figure 20") + 
  theme(axis.text.x = element_text(angle = 90))
```

### Prediction of time-spent
```{r hourlytimespent, echo=FALSE}
years_to_filter_by <- 2016:2019
toll_booth_ff <- unified_ff %>% as_tibble() %>%
  filter (
    year_ %in% years_to_filter_by
    & payment_method %in% c("Automatic", "Cash")
...
lambda <- toll_booth_ff %>%
  # Add this new column to avoid performing string equality in `summarise`.
  mutate (
    is_automatic = ifelse(payment_method == "Automatic", 1, 0)
  ) %>%
  group_by (
    toll_booth_name,
    year_,
  ...
  summarise (
    arrivals = 0.25 * sum(amount),
    arrivals_automatic = 0.25 * sum(amount[is_automatic == 1]),
    arrivals_manual = 0.25 * sum(amount[is_automatic == 0]),
    automatic = sum(amount[is_automatic == 1]) / sum(amount),
    manual = sum(amount[is_automatic == 0]) / sum(amount)
  )
...
rho <- data.frame (
  ...
  day = lambda$day_,
  week = isoweek(
    ymd(paste0(lambda$year_, "-", lambda$month_, "-", lambda$day_))),
  start_hour = lambda$start_hour,
  lambda = lambda$arrivals,
  mu = mu$service,
  booths = lambda$booths,
  booths_required = ceiling(lambda$arrivals / mu$service),
  utilization = lambda$arrivals / (lambda$booths * mu$service)
)

# Used in Chart 3.
# gt <- rho %>%
#   filter(utilization >= 1) %>%
#   group_by(toll_booth_name) %>%
#   summarise(x = n())
# a <- rho %>%
#   group_by(toll_booth_name) %>%
#   summarise(x = n())
# data.frame (
#   tb = a$toll_booth_name,
#   x = gt$x / a$x
# )

required_more <- rho %>% filter(utilization >= 1)

ggplot(data = required_more,
       aes(x = hour(hms(as.character(required_more$start_hour))))) +
  geom_histogram (
    binwidth = 1,
    aes(y=..density..),
    color = "black",
  ...
  labs(y = "Density",
       x = "Hour of day",
       title = "Distribution of over-demand",
       caption = "Figure 20")

rho_stable <- rho %>% filter(utilization < 1)

data <- rho_stable %>%
  mutate (
    queue_delay = apply (
      rho_stable[, c("booths", "lambda", "mu")], 
      1, 
      function(x) 3600 * Wq(x["booths"], x["lambda"], x["mu"])
    )
  ) %>%
  select (
    toll_booth_name, year, month, day, week, start_hour, queue_delay
  )
...
data <- data %>%
  ...
  summarise (
    queue_delay = median(queue_delay)
  )

# A stationary time series is one whose properties do not depend on the time at 
# which the series is observed. if {y_t} is a stationary time series, then for 
# all `s`, the distribution of  (y_t, … , y_{t+s}) does not depend on t. It's of 
# interest for future forecasts to know if the queue_delay time-series is 
# stationary.
qd_ts <- data$queue_delay %>%
  forecast::msts(seasonal.periods = c(365.25))
...
# The test statistic is much bigger than the 1% critical value, indicating the 
# null hypothesis is rejected. That is, the data are not stationary. We can 
# difference the data, and apply the test again.
qd_ts %>% urca::ur.kpss() %>% summary()
# The statistic value is tiny, within the range expected for stationary data.
qd_ts %>% diff() %>% urca::ur.kpss() %>% summary()
# Validate the interpretation with `ndiffs()`. 0 is yielded again.
qd_ts %>% diff() %>% forecast::ndiffs()
# Validate if there's a need for seasonal differencing. 0 is yielded = no diff.
qd_ts %>% diff() %>% forecast::nsdiffs()
# In summary, there is no need to compute a first difference, or a seasonal
# difference.

# Perform forecast and display forecasted values.
qd_ts %>% auto.arima(d = 1, D = 1) %>% forecast() %>% autoplot()
```
