...
rho <- 0.9
# Arrival rate.
lambda <- mean(avellaneda.vehicles.per.hour$vehicles)
...
avellaneda.payments.per.hour <- unified.ff %>% as_tibble() %>%
  filter((Y >= 2014 & Y <= 2018) 
         & toll.booth == 'AVELLANEDA'
         & payment.method %in% c('AUPASS', 'EFECTIVO')) %>%
  select(start.hour, payment.method, amount) %>%
  group_by(start.hour, payment.method) %>%
  summarise(
    vehicles = sum(amount) / (365 * 4)
  )

automatic.payments <- avellaneda.payments.per.hour %>% 
  group_by(start.hour) %>% 
  summarise(
    percentage.of.automatic = 
      sum(vehicles[payment.method == 'AUPASS']) / sum(vehicles)
  )
```
