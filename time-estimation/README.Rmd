...
payments <- unified_ff %>% as_tibble() %>%
  filter (
    (year_ >= 2014 & year_ <= 2018)
    & toll_booth_name == "AVELLANEDA"
...
vehicles <- payments %>%
  group_by (start_hour) %>%
  summarise (
    volume_per_hour = sum(amount) / (365 * 4)
  )
...
automatic_payments <- payments %>%
  group_by(start_hour) %>%
  summarise(
    percentage_of_automatic =
      sum(amount[payment_method == "Automatic"]) / sum(amount)
  )
```

Although this number may seem high, it's important to mention that it includes the impact from automatic toll booths, which can service more vehicles that manual toll booths. For the concrete case of the _Avellaneda_ toll booth plaza, automatic payments constitute `r round(mean(automatic_payments$percentage_of_automatic) * 100, 2)`% of the payments volume. The specific percentages for each our are detailed in Figure 5.

```{r percentagegraph, echo=FALSE}
ggplot(data = automatic_payments,
       aes(x = start_hour, y = percentage_of_automatic, group = 1)) +
  geom_line(linetype = "dashed") +
...
payments_per_hour <- payments %>%
  group_by(start_hour, payment_method) %>%
  summarise(
    volume = sum(amount) / (365 * 4)
  )

lambda.manual <- mean(
  payments_per_hour$volume[payments_per_hour$payment_method == "Cash"])
lambda.automatic <- mean(
  payments_per_hour$volume[payments_per_hour$payment_method == "Automatic"])
...
Traffic intensity $\rho$ can be used to determine the probability of having no vehicles queuing in the system at a given moment of time, and is written as $P_0 = 1 - \rho \; \forall \rho < 1$. This definition of $P_0$ is intuitive, given that if we assume an utilization of $90\%$ it should be expected that the remaining $10\%$ of the time the system is idle.

Given this definition of $P_0$, the probability of having $n$ units in the system would be $P_n = \frac{\rho^n}{n!}P_0$ when $n \leq N$, where $n$ is the amount of vehicles and $N = S$, the amount of servers/toll booths. On the other hand $P_n = \frac{\rho^n}{N^{n-N}N!}P_0$ when $n \geq N$. Depending on the amount of vehicles arriving at the toll booth plaza, one would use the first $P_n$ formula or the latter.
...
prob_n <- function(n, N, rho) {
  P_0 <- 1 - rho
  if (n < N) {
    P_n <- P_0 * (rho^n / factorial(n))
  } else {
    P_n <- P_0 * (rho^n / (N^(n - N) * factorial(N)))
  }
  return(P_n)
}
