...
# Filtering only for Alberti information.
toll_booth_alberti <- toll_booth_ff %>% as_tibble() %>%
  filter (
...
rm(years_to_filter_by, payments_per_hour_alberti, amount_of_days)
```
...
rm(avg_serving_time_automatic, avg_serving_time_manual)
```
...
increase_diff <- data.frame (
  year = 2009:2019,
  difference = diff(general_trend$price)
)

ggplot(data = increase_diff,
