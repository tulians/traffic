...
```
In a previous section it was shown that the highest utilization of the _Alberti_ toll booth plaza happens during the interval of time between 10am and 11am, with a percentage of utilization $\rho$ of 73.5%. That number was obtained from $\rho = \frac{\lambda}{S\mu} = \frac{838.45161}{5 * 228.1761} = 0.73491616$. For this particular set of metric values, the average length of the queue $L_q$ would be `r round(metrics$Lq, 2)` vehicles, the average time spent in a queue $W_q$ would be `r round(metrics$Wq, 2)` seconds, the probability of arriving at an empty queue $P_0$ would be `r round(metrics$P0, 2)`%, and the probability of having to wait to go throught he toll booth $P_d$ would be `r round(metrics$PD, 2)`%, for the mentioned utilization of 73.5%.
