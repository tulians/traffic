...
payments <- unified.ff %>% as_tibble() %>%
  filter (
    ...
    & payment.method %in% c('AUPASS', 'EFECTIVO')
  )
...
automatic.payments <- payments %>% 
  ...
  summarise(
    percentage.of.automatic = 
      sum(amount[payment.method == 'AUPASS']) / sum(amount)
  )
```
...
lambda.manual <- mean(
  payments.per.hour$volume[payments.per.hour$payment.method == 'EFECTIVO'])
lambda.automatic <- mean(
  payments.per.hour$volume[payments.per.hour$payment.method == 'AUPASS'])
