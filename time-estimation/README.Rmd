...
packages.to.include <- c(
  'dplyr', 'tibble', 'ggplot2', 'ff', 'reshape2'
)

lapply(
  packages.to.include, # From this list ...
...
# while.
options(
  fftempdir = file.path(datasets.dir, 'tmp')
)
unified.ff <- read.csv.ffdf(
  file = file.path(datasets.dir, 'unified.csv')
)
rm(datasets.dir)
```
...
ggplot(data = oil_price, 
       aes(x = year_, y = price, group = oil.type)) +
...
ff.df.differences <- data.frame(
  year_ = diff(ff.df$year_),
  month_ = diff(ff.df$month_),
...
The next two subsections will be focused on the _Avellaneda_ toll booth plaza given that it's the toll booth with the highest volume of vehicles, and with an average of 33 toll booths. The amount of vehicles that arrive on each hour to the toll booth, as shown in Figure 4, is computed considering only the information from January 2014 to December 2018, due to the [asymmetries in the dataset](https://github.com/tulians/traffic/tree/master/descriptive#increment-in-traffic).

```{r avellanedavehiclesperhour, echo=FALSE}
payments <- unified.ff %>% as_tibble() %>%
  filter (
    (year_ >= 2014 & year_ <= 2018) 
    & toll.booth.name == 'AVELLANEDA'
    & payment.method %in% c('Automatic', 'Cash')
  )

vehicles <- payments %>%
  group_by (start.hour) %>%
  summarise (
    volume.per.hour = sum(amount) / (365 * 4)
  )
...
rho <- 0.9
# Arrival rate.
lambda <- mean(vehicles$volume.per.hour)
# Service rate per toll booth has to be greater than \lambda / (S * \rho).
mu <- lambda / (S * rho)

ggplot(data = vehicles, 
       aes(x = start.hour, y = volume.per.hour, group = 1)) +
...
An initial approach to the estimation of time-spent on queues would be to assume that toll booths are operating at 90% of their capacity. It actually makes sense to think that toll booth plazas were designed to be a stable system, due to the fact that even though long queues happen, and frequently during peak hours, they are eventually served up to the point that there's no queue left. In this scenario, each toll booth at the _Avellaneda_ toll booth plaza is able to serve up to `r mu` vehicles in an hour.

where $\lambda$ is equal to the `r round(lambda, 2)` vehicles per hour, the average of the amount of vehicles per hour shown in Figure 4, the number of servers $S$ is `r S`, and the utilization/traffic intensity $\rho$ is $90\%$. This result means that in order for the system (toll booth plaza) to be operating at 90% of its capacity each server (toll booth) needs to service at least `r round(mu, 2)` vehicles per hour. Summing up each server contribution, `r round(S * mu, 2)` vehicles can be serviced in an hour by this system. Servicing `r round(mu, 2)`  vehicles in an hour means that on average `r round(mu / 60, 2)` vehicles are serviced per minute. 

```{r effectofautomatictollbooths, echo=FALSE}
automatic.payments <- payments %>% 
  group_by(start.hour) %>% 
  summarise(
    percentage.of.automatic = 
      sum(amount[payment.method == 'Automatic']) / sum(amount)
  )
```

Although this number may seem high, it's important to mention that it includes the impact from automatic toll booths, which can service more vehicles that manual toll booths. For the concrete case of the _Avellaneda_ toll booth plaza, automatic payments constitute `r round(mean(automatic.payments$percentage.of.automatic) * 100, 2)`% of the payments volume. The specific percentages for each our are detailed in Figure 5.

```{r percentagegraph, echo=FALSE}
ggplot(data = automatic.payments,
       aes(x = start.hour, y = percentage.of.automatic, group = 1)) +
  geom_line(linetype = 'dashed') +
  geom_point(color = 'red') +
  labs(x = 'Hour of day', 
       y = 'Percentage of automatic payments',
       title = 'Percentage of vehicles going through automatic toll booths', 
       caption = 'Figure 5') +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

```{r breakdownperpaymentmethod, echo=FALSE}
payments.per.hour <- payments %>%
  group_by(start.hour, payment.method) %>%
  summarise(
    volume = sum(amount) / (365 * 4)
  )

lambda.manual <- mean(
  payments.per.hour$volume[payments.per.hour$payment.method == 'Cash'])
lambda.automatic <- mean(
  payments.per.hour$volume[payments.per.hour$payment.method == 'Automatic'])

# The Avellaneda toll booth has at least 6 automatic payment toll booths.
automatic.tolls <- 6

mu.manual <- lambda.manual / (S - automatic.tolls * rho)
mu.automatic <- lambda.automatic / (automatic.tolls * rho)
```

If we were to break down the service rate per payment method, we should expect to see faster service time for automatic toll booths, that could account for a high percentage of the almost `r round(mu / 60, 2)` vehicles served per minute. Performing such break down yields `r mu.manual / 60` vehicles services by minute in manual payment toll booths, and `r mu.automatic / 60` vehicles services by minute in automatic payment toll booths.

###### Characterization of the M/M/S system
Previously a traffic intensity $\rho$ of $90\%$ was assumed, and by doing so a realistic service rate `r round(mu.manual / 60, 2)` vehicles per minute in manual toll booths and `r round(mu.automatic / 60, 2)` vehicles per minute in automatic toll booths was obtained.
...
$$
  P_0 = 1 - \rho \quad \forall \rho < 1
$$

This definition of $P_0$ is intuitive, given that if we assume an utilization of $90\%$ it should be expected that the remaining $10\%$ of the time the system is idle. $P_0$ can be used to define the probability of having $n$ vehicles in the system

$$
  P_n = \frac{\rho^n}{n!}P_0 \quad \forall n \leq N
$$

and 

$$
  P_n = \frac{\rho^n}{N^{n-N}N!}P_0 \quad \forall n \geq N
$$