...
The next two subsections will be focused on the *Alberti* toll booth plaza given that it's a toll booth from which we count with real information about service time ![\\mu](https://latex.codecogs.com/png.latex?%5Cmu "\mu"). The amount of vehicles that arrive on each hour to the toll booth, as shown in Figure 4, is computed considering recent information from year 2019. Additionally, only the *cars* volume will be considered for the mentioned sections, as the analysis can be then extended to the least frequent vehicle types.

![](README_files/figure-markdown_github/albertivehiclesperhour-1.png)

##### Assumption of 90% utilization

An initial approach to the estimation of time-spent on queues would be to assume that toll booths are operating at 90% of their capacity. It actually makes sense to think that toll booth plazas were designed to be a stable system, due to the fact that even though long queues happen, and frequently during peak hours, they are eventually served up to the point that there's no queue left. In this scenario, each toll booth at the *Alberti* toll booth plaza is able to serve up to 398.0833333 vehicles in an hour. This figure comes from computing ![\\mu = \\frac{\\lambda}{S\\rho}](https://latex.codecogs.com/png.latex?%5Cmu%20%3D%20%5Cfrac%7B%5Clambda%7D%7BS%5Crho%7D "\mu = \frac{\lambda}{S\rho}") where ![\\lambda](https://latex.codecogs.com/png.latex?%5Clambda "\lambda") is equal to the 1791.38 vehicles per hour, the average of the amount of vehicles per hour shown in Figure 4, the number of servers ![S](https://latex.codecogs.com/png.latex?S "S") is 5, and the utilization/traffic intensity ![\\rho](https://latex.codecogs.com/png.latex?%5Crho "\rho") is ![90\\%](https://latex.codecogs.com/png.latex?90%5C%25 "90\%"). This result means that in order for the system (toll booth plaza) to be operating at 90% of its capacity, each server (toll booth) needs to service at least 398.08 vehicles per hour. Summing up each server contribution, 1990.42 vehicles can be serviced in an hour by this system. Servicing 1990.42 vehicles in an hour means that on average 33.17 vehicles are serviced per minute in a toll booth plaza, regardless of the toll booth payment method.

Although this number may seem high, it's important to mention that it includes the impact from automatic toll booths, which can service more vehicles that manual toll booths. For the concrete case of the *Alberti* toll booth plaza, automatic payments constitute 36.14% of the payments volume. The specific percentages for each our are detailed in Figure 5.

![](README_files/figure-markdown_github/utilizationbreakdown-1.png)

If we were to break down the service rate per payment method, we should expect to see faster service time for automatic toll booths, that could account for a high percentage of the almost 33.17 vehicles served per minute. Performing such break down yields NaN vehicles services by minute in manual payment toll booths, and 12.16 vehicles services by minute in automatic payment toll booths. Even though the service rate for manual toll booths is higher than that of automatic toll booths, it must be taken into account that there are more toll booths with manual payment methods that booths with automatic ones. For the particular case of the *Alberti* toll booth plaza, an approximate of 5 toll booths are automatic, from a total of 5. This shows that even with just 100% of the toll booths being automatic in this plaza, you can service 36.14% of the incoming traffic, as it was mentioned previously.

###### Characterization of the M/M/S system

Previously a traffic intensity ![\\rho](https://latex.codecogs.com/png.latex?%5Crho "\rho") of ![90\\%](https://latex.codecogs.com/png.latex?90%5C%25 "90\%") was assumed, and by doing so a realistic service rate NaN vehicles per minute in manual toll booths and 12.16 vehicles per minute in automatic toll booths was obtained.
...
Traffic intensity ![\\rho](https://latex.codecogs.com/png.latex?%5Crho "\rho") can be used to determine the probability of having no vehicles queuing in the system at a given moment of time, and is written as ![P\_0 = \\frac{1}{\\sum\_{n = 0}^{N - 1}\\frac{\\rho^n}{n!}+\\frac{\\rho^N}{N!(1-\\frac{\\rho}{N})}}](https://latex.codecogs.com/png.latex?P_0%20%3D%20%5Cfrac%7B1%7D%7B%5Csum_%7Bn%20%3D%200%7D%5E%7BN%20-%201%7D%5Cfrac%7B%5Crho%5En%7D%7Bn%21%7D%2B%5Cfrac%7B%5Crho%5EN%7D%7BN%21%281-%5Cfrac%7B%5Crho%7D%7BN%7D%29%7D%7D "P_0 = \frac{1}{\sum_{n = 0}^{N - 1}\frac{\rho^n}{n!}+\frac{\rho^N}{N!(1-\frac{\rho}{N})}}"), where ![N](https://latex.codecogs.com/png.latex?N "N") is the number of toll booths ![S](https://latex.codecogs.com/png.latex?S "S"). Given this definition of ![P\_0](https://latex.codecogs.com/png.latex?P_0 "P_0"), the analogoues ![P\_n](https://latex.codecogs.com/png.latex?P_n "P_n"), which would be the probability of having ![n](https://latex.codecogs.com/png.latex?n "n") units in the system, is defined as ![P\_n = \\left ( \\frac{\\rho}{n} \\right ) P\_{n-1}](https://latex.codecogs.com/png.latex?P_n%20%3D%20%5Cleft%20%28%20%5Cfrac%7B%5Crho%7D%7Bn%7D%20%5Cright%20%29%20P_%7Bn-1%7D "P_n = \left ( \frac{\rho}{n} \right ) P_{n-1}"). With our assumption of ![90\\%](https://latex.codecogs.com/png.latex?90%5C%25 "90\%") utilization the probability of having no vehicles in the system is 40.65, while the probability of having ![S](https://latex.codecogs.com/png.latex?S "S") vehicles arriving at the ![S](https://latex.codecogs.com/png.latex?S "S") toll booths at the exact same time to be served is 36.59.

###### Average number of units in queues

Using the probability of having an empty queue ![P\_0](https://latex.codecogs.com/png.latex?P_0 "P_0") we can derive the average queue length. This metric will then be used to estimate the average waiting time in queues. As this are average times, they will take into account those times where there are no vehicles queuing, and times where there are long queues. For the case of ![S](https://latex.codecogs.com/png.latex?S "S") servers, the average queue length is defined as ![U\_q(N, \\rho) = P\_0\\frac{\\rho^{N+1}}{N!N}\\left \[ \\frac{1}{\\left (1 - \\frac{\\rho}{N} \\right )^2} \\right \]](https://latex.codecogs.com/png.latex?U_q%28N%2C%20%5Crho%29%20%3D%20P_0%5Cfrac%7B%5Crho%5E%7BN%2B1%7D%7D%7BN%21N%7D%5Cleft%20%5B%20%5Cfrac%7B1%7D%7B%5Cleft%20%281%20-%20%5Cfrac%7B%5Crho%7D%7BN%7D%20%5Cright%20%29%5E2%7D%20%5Cright%20%5D "U_q(N, \rho) = P_0\frac{\rho^{N+1}}{N!N}\left [ \frac{1}{\left (1 - \frac{\rho}{N} \right )^2} \right ]"), and has an approximate value of 5.355123910^{-4}. This value is actually very small, and could be rounded to ![0](https://latex.codecogs.com/png.latex?0 "0"), and can be interpreted as arriving vehicles not having to queue for going through a toll booth.

###### Average waiting time for units in queues

The ![U\_q(N,\\rho)](https://latex.codecogs.com/png.latex?U_q%28N%2C%5Crho%29 "U_q(N,\rho)") metric can be reused to define the time spent waiting on queues as ![W\_q(N,\\rho,\\lambda) = U\_q/\\lambda](https://latex.codecogs.com/png.latex?W_q%28N%2C%5Crho%2C%5Clambda%29%20%3D%20U_q%2F%5Clambda "W_q(N,\rho,\lambda) = U_q/\lambda"). The units of this metric are hours, and for this particular case it's value is 2.98939310^{-7}, again very close to ![0](https://latex.codecogs.com/png.latex?0 "0"). This result goes in tandem with the results from the average number of vehicles per queue, as not having to queue behind other vehicles means not spending that time on queues.
...
The two previous calculations of average number of units in queues and average waiting time in queues shown that on average vehicles shouldn't wait for going through toll booths. However, there are moments of time during the day (peak hours) were it's a fact that's not the case.
    ...
    ## Warning in adf.test(y, alternative = "stationary"): p-value smaller than
    ## printed p-value
    ...
    ## 
    ## data:  y
    ## Dickey-Fuller = -5.0916, Lag order = 4, p-value = 0.01
    ## alternative hypothesis: stationary

#### Estimation of service rate from empiric measurements

In the previous section the value of ![\\mu](https://latex.codecogs.com/png.latex?%5Cmu "\mu") derived after assuming 90% utilization of the system. In order to know if the system is actually performing at such percentage or if it's actually a higher or lower one, empiric measurements in real toll booths were performed.
