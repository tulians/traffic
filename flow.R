# Traffic flow analysis in AUSA tolls.
...
#### Input data ####
# Files from 2008 to 2018 have different column structure.
...
# awk 'FNR==1 && NR!=1{next;}{print}' *.csv > ~/Documents/final/datasets/merged.csv
system('awk "FNR==1 && NR!=1{next;}{print}" ./datasets/flujo-vehicular-por-unidades-de-peaje-ausa/*.csv > ./datasets/merged.csv')
merged <- './datasets/merged.csv'
traffic <- './datasets/traffic.csv'
df <- read.csv(merged, sep = ';', header = T, stringsAsFactors = T)
...
# Once the data wrangling process is completed, save the transformed dataframe 
# into a new .csv file and use that one from now on.
write.csv(df, file = traffic, row.names = F)
rm(df, merged, traffic)

#### Perform EDA on the traffic dataset ####
df <- read.csv('./datasets/traffic.csv', header = T, stringsAsFactors = T)
